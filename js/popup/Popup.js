		...
		function threadsOfActiveMode( mode, srtn, callback ){
			...
			else if (mode == "image")  	
			{
				...
				if ( GetThemAll.Prefs.get( "gta.filter_image_all" ) == "false" )
				{
					...
					var flag_webp  = _b(GetThemAll.Prefs.get( "gta.filter_image_webp" ));
					...
					media = media.filter( function (item, i, arr) {
											...
											if ( flag_webp && item.ext == "webp" ) return true;											
		...
		function buildThreadItem( media, mode ){
			...
			if (media.type == "image")	{
				switch ( media.ext )	{
					...
					case "webp":   extClass = "gta_downloader_icon_ic_list_webp";   break;					
